...
void DM_Device_CAN1_Callback(Struct_CAN_Rx_Buffer *CAN_RxMessage)
{
    ...
    case (0xA1):
    {
        dm_motor.CAN_RxCpltCallback(CAN_RxMessage->Data);
    }
    break;
    case (0x201):
    {
...
{   
    ...
    //dm_motor
  //  dm_motor.TIM_Process_PeriodElapsedCallback(); //发送电机数据
    count++;
    if(count==50) //每50ms查询，电机是否存活
    {   
...
{  
    ...
    /********************************** 驱动层初始化 **********************************/
        ...
        //dm_motor初始化(位置速度模式)
        dm_motor.Init(&hcan1,DM_Motor_ID_0xA1,DM_Motor_Control_Method_POSITION_OMEGA);
				dm_motor.Set_DM_Control_Status(DM_Motor_Control_Status_ENABLE);
        //集中总线can1/can2
        CAN_Init(&hcan1, DM_Device_CAN1_Callback);  //配置dm_motor的can接收回调函数
 ...
 extern "C" void Task_Loop()
{
   ...
   /*    float now_angle_yaw = chariot.Gimbal.Motor_Yaw.Get_True_Angle_Yaw();
        ...
        dm_motor.Set_Target_Omega(1); //设置速度
        dm_motor.Set_Target_Angle(40);//设置位置值
