...
KalmanFilter KF[3];// PITCH YAW ROLL
...
{   
    //更新卡尔曼滤波数据
    kalman_update(&KF[0],chariot.Gimbal.Boardc_BMI.Get_Angle_Pitch());//pitch
    kalman_update(&KF[1],chariot.Gimbal.Boardc_BMI.Get_Angle_Yaw());//yaw   
    kalman_update(&KF[2],chariot.Gimbal.Boardc_BMI.Get_Angle_Roll());//roll

    //imu
  //  chariot.Gimbal.Boardc_BMI.TIM_Calculate_PeriodElapsedCallback();

    count++;
    if(count==50) //每50ms查询，imu是否存活
    {   
        chariot.Gimbal.Boardc_BMI.TIM1msMod50_Alive_PeriodElapsedCallback();
        count=0;
    }
...
{  
    ...
    /********************************** 驱动层初始化 **********************************/
        ...
        chariot.Gimbal.Boardc_BMI.Init();
