...
//DM_MOTOR
Class_DM_Motor_J4310 dm_moto_0xf1,dm_moto_0xf2;
 ...
 */
#ifdef GIMBAL
void DM_Device_CAN1_Callback(Struct_CAN_Rx_Buffer *CAN_RxMessage)
{
    switch (CAN_RxMessage->Header.StdId)
    {
    case (0xA1):
    {
        dm_moto_0xf1.CAN_RxCpltCallback(CAN_RxMessage->Data);
    }
    break;
    case (0x201):
    {
      //  dm_moto_0xf2.CAN_RxCpltCallback(CAN_RxMessage->Data);
    }
    break;
    case (0x202):
    {
...
{   
    ...
    //dm_moto_0xf1
    dm_moto_0xf1.TIM_Process_PeriodElapsedCallback(); //发送电机数据
 //   dm_moto_0xf2.TIM_Process_PeriodElapsedCallback(); //发送电机数据
    count++;
    if(count==3) //每3ms查询，电机是否存活
    {   
        dm_moto_0xf1.TIM_Alive_PeriodElapsedCallback(); //判断电机是否存活
   //     dm_moto_0xf2.TIM_Alive_PeriodElapsedCallback(); //判断电机是否存活
        count=0;
    }
...
{  
    ...
    /********************************** 驱动层初始化 **********************************/
        ...
        //集中总线can1/can2
        CAN_Init(&hcan1, DM_Device_CAN1_Callback);  //配置dm_moto_0xf1的can接收回调函数
        CAN_Init(&hcan2, Gimbal_Device_CAN2_Callback);

        //dm_moto_0xf1初始化(位置速度模式)
        dm_moto_0xf1.Init(&hcan1,DM_Motor_ID_0xA1,DM_Motor_Control_Method_POSITION_OMEGA);
        dm_moto_0xf1.Set_DM_Control_Status(DM_Motor_Control_Status_ENABLE);

        //dm_moto_0xf2初始化(位置模式)
//				dm_moto_0xf2.Init(&hcan1,DM_Motor_ID_0xA2,DM_Motor_Control_Method_MIT_POSITION);
//				dm_moto_0xf2.Set_DM_Control_Status(DM_Motor_Control_Status_ENABLE);
        //c板陀螺仪spi外设
 ...
 extern "C" void Task_Loop()
{
   ...
   /*    float now_angle_yaw = chariot.Gimbal.Motor_Yaw.Get_True_Angle_Yaw();
        ...
        dm_moto_0xf1.Set_Target_Omega(1); //设置速度
        dm_moto_0xf1.Set_Target_Angle(t_angle);//设置位置值
